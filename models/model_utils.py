...
class FrustumPointNetLoss(nn.Module):
    ...
    def forward(self, logits, mask_label, \
        ...
        heading_residual_normalized_loss = huber_loss(
            heading_residual_normalized_dist - heading_residual_normalized_label, delta=1.0)  ###fix,2020.1.14
        ...
        predicted_size_residual_normalized_dist = torch.sum( size_residual_normalized * scls_onehot_repeat.cuda(), dim=1)  # 32,3
        mean_size_arr_expand = torch.from_numpy(g_mean_size_arr).float().cuda().view(1, NUM_SIZE_CLUSTER, 3)  # 1,8,3
        ...
        size_normalized_dist = torch.norm(size_residual_label_normalized - predicted_size_residual_normalized_dist, dim=1)  # 32
        size_residual_normalized_loss = huber_loss(size_normalized_dist,
                                                   delta=1.0)  # tensor(11.2784, grad_fn=<MeanBackward0>)

        # Corner Loss
        corners_3d = get_box3d_corners(center, heading_residual, size_residual).cuda()  # (bs,NH,NS,8,3)(32, 12, 8, 8, 3)
        ...
        corners_3d_pred = torch.sum( gt_mask.view(bs, NUM_HEADING_BIN, NUM_SIZE_CLUSTER, 1, 1).float().cuda() * corners_3d, dim=[1, 2])  # (bs,8,3)
        heading_bin_centers = torch.from_numpy(np.arange(0, 2 * np.pi, 2 * np.pi / NUM_HEADING_BIN)).float().cuda()  # (NH,)
        heading_label = heading_residual_label.view(bs, 1) + heading_bin_centers.view(1, NUM_HEADING_BIN)  # (bs,1)+(1,NH)=(bs,NH)
