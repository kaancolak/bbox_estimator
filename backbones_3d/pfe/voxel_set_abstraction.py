...
class VoxelSetAbstraction(nn.Module):
    ...
    def get_sampled_points(self, batch_dict):
        ...
        if self.model_cfg.SAMPLE_METHOD == 'HVCS':
            if not hasattr(self, 'hvcs2'):
                from ....ops.hvcs import hvcs_cuda as hvcs_ops
                self.hvcs2_stack = hvcs_ops.potential_voxel_size_stack
            cur_pt_idxs = self.hvcs2_stack(batch_indices.contiguous(), src_points.contiguous(), batch_size,
                                           4096, [0.5, 0.5, 0.35], 0.01, 20)
            keypoints = src_points[cur_pt_idxs].view(batch_size, -1, 3)
        else:
            keypoints_list = []
